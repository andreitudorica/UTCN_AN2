...
int l(int x1, int  y1, int  x2, int  y2, int  i, int j)
{
	return (y1 - y2)*i + (x2 - x1)*j + x1*y2 + x2*y1;
}

void renderTriangle(int x1, int y1, SDL_Color c1, int x2, int y2, SDL_Color c2, int x3, int y3, SDL_Color c3)
{
	for (int i = min(x1, min(x2, x3)); i <= max(x1, max(x2, x3)); i++)
		for (int j = min(y1, min(y2, y3)); j <= max(y1, max(y2, y3)); j++)
		{
			//std::cout << min(x1, min(x2, x3));
			int l13x = (y1 - y3)*i + (x3 - x1)*j + x1*y3 - x3*y1;
			int l13x1 = (y1 - y3)*x2 + (x3 - x1)*y2 + x1*y3 - x3*y1;

			int l23x = (y2 - y3)*i + (x3 - x2)*j + x2*y3 - x3*y2;
			int l23x1 = (y2 - y3)*x1 + (x3 - x2)*y1 + x2*y3 - x3*y2;
			double alpha = (double)l23x / (double)l23x1;
			double beta = (double)l13x / (double)l13x1;
			double gamma = 1 - alpha - beta;

			//std::cout << alpha<<" "<<beta<<" "<<gamma<<'\n' ;
			if (alpha <= 1 && beta <= 1 && gamma <= 1 &&  alpha >= 0 && beta >=0 && gamma >= 0)
			{
				SDL_SetRenderDrawColor(renderer, c1.r*alpha + c2.r*beta + c3.r*gamma, c1.g*alpha + c2.g*beta + c3.g*gamma, c1.b*alpha + c2.b*beta + c3.b*gamma, 0x00);
...
int main(int argc, char * argv[]) {
	...
	while (!quit) {
		...
		while (SDL_PollEvent(&currentEvent) != 0)
		{
			...
			c2 = { 0,255,0 };
			c3 = { 0,0,255 };
			renderTriangle(100, 100, c1, 300, 300, c2, 100, 300, c3);
