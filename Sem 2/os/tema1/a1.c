...
void list(const char *path, int recursive, int name_starts_with, const char *starts_with, int has_perm_write, int lvl)
{
	...
	while (d != NULL)
	{
		...
		if (recursive)
		    list(file_path, recursive, name_starts_with, starts_with, has_perm_write, 1);
		if ((!name_starts_with && !has_perm_write) || ((name_starts_with && !strncmp(d->d_name, starts_with, strlen(starts_with))) && !has_perm_write) || (!name_starts_with && has_perm_write && access(file_path, W_OK) == 0))
		    printf("%s\n", file_path);
		if (file_path != NULL)
		    free(file_path);
	    }
...
{
    char section_names[16][13], magic[2];
    ...
    read(file, &hsize, 2);
    read(file, &version, 2);
    if (version < 44 || version > 163)
	errors = wversion = 1;
    read(file, &section_count, 1);
    if (section_count < 4 || section_count > 15)
	errors = wsection_count = 1;
	...
	if (section_types[i] != 37 && section_types[i] != 74 && section_types[i] != 97 && section_types[i] != 11 && section_types[i] != 16 && section_types[i] != 53 && section_types[i] != 93)
	    errors = wsection_types = 1;
    }
    if (errors)
    {
	if (wmagic)
	    ...
	    printf("ERROR\nwrong version");
	else if (wsection_count)
	    printf("ERROR\nwrong sect_nr");
	else if (wsection_types)
	    printf("ERROR\nwrong sect_types");
    }
    else
    {
...
int main(int argc, char **argv)
{
    if (argc >= 2)
    {
	char *path = NULL, *section = NULL, *line = NULL, *starts_with = NULL;
	int recursive = 0, name_starts_with = 0, has_perm_write = 0, op = 0;
	...
	if (op == 1)
	    list(path, recursive, name_starts_with, starts_with, has_perm_write, 0);
	if (op == 2)
	    parse(path);
	if (op == 3)
